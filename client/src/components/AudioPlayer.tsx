...
const AudioPlayer = ({ 
  ...
  const formattedAudioUrl = audioUrl
  ...
  useEffect(() => {
    ...
    if (!formattedAudioUrl) {
      ...
      setError("No audio URL provided. Please try generating the audio again.");
      return;
    }
            ...
            {error && (
              <p className="text-sm text-red-500 mt-1">
                ...
                {(isEmptyAudio || isFallback) && (
                  <span className="block mt-1 text-xs">
                    The text might be too long or complex. Try regenerating audio.
                  </span>
                )}
              </p>
            )}
