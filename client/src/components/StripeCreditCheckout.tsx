...
export default function StripeCreditCheckout({ packageId, onSuccess, onCancel }: CreditCheckoutProps) {
  const [clientSecret, setClientSecret] = useState("");
  const [isLoading, setIsLoading] = useState(true);
  const { toast } = useToast();

  useEffect(() => {
    // Create PaymentIntent for the selected credit package
    const createPaymentIntent = async () => {
      try {
        setIsLoading(true);
        const response = await apiRequest("POST", "/api/payment/create-credit-checkout", { packageId });
  ...
  if (!clientSecret) {
    return (
      <div className="p-4 text-center">
        <p className="text-destructive">Unable to initialize payment. Please try again.</p>
