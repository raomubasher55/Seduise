...
const PopularStoryItem = ({ story }: PopularStoryItemProps) => {
  ...
  // Get a short preview of the content for the description
    ...
    <Card className="bg-[#1E1E1E] border-0 hover:bg-[#2D2D2D] transition-colors">
      <CardContent className="p-6">
        ...
        <p className="text-sm text-gray-400 mb-4">
          {story.description || getPreviewText(story.content)}
        </p>
        <div className="flex justify-between">
          <div className="flex items-center text-sm text-gray-500">
            <Heart className="h-4 w-4 mr-1 text-[#A93B5B]" /> 
            <span className="mr-4">{story.likes || 0}</span>
            <Play className="h-4 w-4 mr-1" /> 
            <span>{story.plays || 0} plays</span>
          </div>
          <Button 
            variant="outline" 
            size="sm" 
